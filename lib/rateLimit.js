...
export function getClientIp(request) {
  // Vercel computes this header itself from the actual connecting client IP —
  // unlike x-forwarded-for, a client can't inject a value into it, and it
  // stays accurate even if an upstream proxy sits in front of the deployment.
  // https://vercel.com/docs/headers/request-headers#x-vercel-forwarded-for
  const vercelForwarded = request.headers.get("x-vercel-forwarded-for");
  if (vercelForwarded) return vercelForwarded.split(",")[0].trim();

  // Fallbacks for local dev / non-Vercel environments, where the header above isn't set.
...
export async function checkRateLimit(ip, route, maxRequests, windowSeconds) {
  if (isLocalIp(ip)) return { allowed: true, remaining: 999 };
