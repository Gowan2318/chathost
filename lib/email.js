...
export async function sendVoiceMinutesLimitReachedEmail({ businessName, clientId, used, limit }) {
  ...
  const html = emailShell(`
    ...
    <p style="font-size: 14px; color: #718096; margin-top: 16px;">
      The voice assistant has NOT been paused automatically — decide whether to raise their limit or take action.
    </p>
    ${button(SUPABASE_TABLE_URL, "View in Supabase")}
  `);
  ...
  await sendEmail({
    to: FOUNDER_EMAIL,
    subject: `🚫 Voice minutes at 100%: ${businessName || "Unknown business"}`,
    html,
  });
}
