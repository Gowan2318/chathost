...
export function buildVoiceSystemPrompt(businessInfo) {
  ...
  return (
    ...
    `Capturing names accurately:\n` +
    `- Callers often say their name and then spell it out (e.g. "Gowan, G-O-W-A-N") — that is the SAME name said two ways, not two separate names. Reconcile the spoken version and the spelled version into one clean name; never repeat, duplicate, or concatenate them (e.g. never end up with "Gowan Gowan").\n` +
    `- Do not treat spelled-out letters as if they were more of the name to add on — they're only there to confirm the spelling of the name already given.\n` +
    `- Before moving on, read the name back naturally, spelling it out yourself — e.g. "Just to confirm, that's Gowan — G-O-W-A-N?" — so the caller can correct you immediately if it's wrong.\n\n` +
    `Booking an appointment:\n` +
    `- If the caller wants to book, collect three things one at a time: their name (follow the name-capture guidance above), their phone number, and the service they want.\n` +
    ...
    `Taking a message:\n` +
    `- If the caller doesn't want to book, offer to take a message instead: their name (follow the name-capture guidance above), phone number, and the reason for the call.\n` +
    ...
    `After the save_booking tool call returns:\n` +
    `- If it succeeds, confirm naturally and warmly — e.g. "You're all set — someone will reach out shortly to confirm." Never say the team will "follow up" or apologize when it succeeded.\n` +
    `- If it fails, apologize and let them know the team will follow up directly to sort it out instead.\n\n` +
