...
export function validateBusinessName(value) {
  ...
  if (!/^[a-zA-Z0-9\s\-'\.&]+$/.test(name)) {
    return "Use only letters, numbers, spaces, hyphens, apostrophes, periods, and ampersands.";
  }
...
export function validateBusinessDescription(value) {
  ...
  if (t.length > 500) return "Business description cannot exceed 500 characters.";
...
export function validateServicesDescription(value) {
  ...
  if (t.length > 300) return "Services cannot exceed 300 characters.";
...
export function validatePhone(value) {
  ...
  // Accept 10 digits (local) or 11 digits (with country code, e.g. +1)
  if (digits.length < 10 || digits.length > 11) return PHONE_ERROR;
  if (isFakePhoneNumber(digits.slice(-10))) return PHONE_ERROR;
...
export function validateStep(step, form, { plan = "pro" } = {}) {
  ...
  if (step === 2) {
    ...
    // Phone: required unless a booking URL or support email is already provided
    if (form.supportPhone?.trim()) {
      const phoneErr = validatePhone(form.supportPhone);
      if (phoneErr) add("supportPhone", phoneErr);
    } else if (!form.bookingUrl?.trim() && !form.supportEmail?.trim()) {
      add("supportPhone", "Please provide a phone number, email, or booking link so customers can reach you.");
    }

    // Email: always optional, validate format if filled
