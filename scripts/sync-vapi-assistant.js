...
async function run(clientId) {
  ...
  // assistant's current model config first so provider/model/temperature/etc.
  // are preserved, and only the system message is swapped out.
  let currentModel;
  try {
  ...
  if (!response.ok) {
    ...
    return `Vapi returned ${response.status} ${response.statusText} — ${text}`;
  }
