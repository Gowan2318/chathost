...
const STEPS = [
  { step: "1", title: "Build your chatbot in the builder (15 minutes)" },
  { step: "2", title: "Copy your embed code" },
  { step: "3", title: "Paste it on your website" },
  { step: "4", title: "Your AI assistant goes live!" },
];
...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/widget?id=${encodeURIComponent(clientId)}`)
      ...
      .then((config) => setBusinessName(config.businessName || ""))
      .catch(() => {});
  }, [clientId]);
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      <main className="mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center px-6 py-16 text-center">
        ...
        <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-[#0D7377] to-[#14A3A8] shadow-xl shadow-[#0D7377]/30">
          <svg
        ...
        <p className="mx-auto mt-5 max-w-lg text-base leading-relaxed text-[#4A5568] sm:text-lg">
        ...
        {clientId ? (
          <div className="mt-10 w-full max-w-md space-y-4 text-left">
            <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold text-[#0D7377]">What does this code do?</p>
          ...
          <p className="mt-10 text-sm text-[#4A5568]">
            ...
            <a href="mailto:support@vestachathost.com" className="text-[#0D7377] hover:text-[#0A5D61]">
        ...
        <div className="mt-14 w-full max-w-md rounded-2xl border border-[#E2E8F0] bg-white p-8 text-left shadow-sm">
          <p className="mb-6 text-center text-sm font-semibold uppercase tracking-widest text-[#0D7377]">
            What&apos;s next
          </p>
          <ol className="space-y-5">
            {STEPS.map((item) => (
              <li key={item.step} className="flex items-start gap-4">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#0D7377]/10 text-sm font-bold text-[#0D7377]">
                  {item.step}
                </span>
                <p className="pt-1 text-sm leading-relaxed text-[#4A5568] sm:text-base">
                  <span className="font-medium text-[#1A1A2E]">Step {item.step}:</span> {item.title}
                </p>
              </li>
            ))}
          </ol>
        ...
        <p className="mt-12 text-sm text-[#4A5568]">
          Need help?{" "}
          <a
            href="mailto:support@vestachathost.com"
            className="font-medium text-[#0D7377] transition hover:text-[#0A5D61]"
          >
            support@vestachathost.com
          </a>
        </p>
